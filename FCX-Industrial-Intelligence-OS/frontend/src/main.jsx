...
const navigation = [
  ...
  { path: '/alarms', label: 'Alarms', icon: TriangleAlert },
  { path: '/work-orders', label: 'Work Orders', icon: ClipboardList },
  { path: '/integrations', label: 'Integrations', icon: Activity },
  { path: '/agent-runtime', label: 'Agent Runtime', icon: Workflow },
];
...
function App() {
  ...
  const page = {
    ...
    '/agent-runtime': <AgentRuntimePage />,
  }[route] || <DashboardPage />;
...
function AgentRuntimePage() {
  ...
  const { data, loading, error } = useApi('/api/agent-runtime/dashboard', fallback);

  return (
    <>
      <PageHeader title="Agent Runtime" subtitle="Execucao local, fila, agentes e resultados do FCX Agent Operating System." />
      <StatusLine loading={loading} error={error} />
      <section className="kpiGrid">
        <Kpi label="Workflows executados" value={data.metrics?.workflowsExecuted || 0} />
        <Kpi label="Workflows concluidos" value={data.workflows?.completed || 0} />
        <Kpi label="Tarefas executadas" value={data.metrics?.tasksExecuted || 0} />
        <Kpi label="Falhas" value={data.metrics?.tasksFailed || 0} tone="danger" />
        <Kpi label="Fila pendente" value={data.queue?.queued || 0} tone="warning" />
        <Kpi label="Agentes ativos" value={(data.metrics?.activeAgents || []).length} />
      </section>
      <section className="widgetGrid">
        <Widget title="Agent Registry">
          <div className="eventList">
            {(data.agents || []).map((agent) => (
              <div key={agent.id}>
                <strong>{agent.id}</strong>
                <span>{agent.status} - {(agent.capabilities || []).join(', ')}</span>
              </div>
            ))}
          </div>
        </Widget>
        <Widget title="Execution Queue">
          <div className="eventList">
            <div><strong>{data.queue?.completed || 0} concluidas</strong><span>{data.queue?.running || 0} executando</span></div>
            <div><strong>{data.queue?.queued || 0} aguardando</strong><span>{data.queue?.failed || 0} falhas</span></div>
          </div>
        </Widget>
        <Widget title="Execution Reports">
          <div className="eventList">
            {(data.reports || []).slice(-8).reverse().map((report) => (
              <div key={report.workflow?.workflowId}>
                <strong>{report.workflow?.name}</strong>
                <span>{report.workflow?.status} - {(report.results || []).length} tarefas</span>
              </div>
            ))}
            {(data.reports || []).length === 0 ? <span>Nenhuma execucao registrada.</span> : null}
          </div>
        </Widget>
      </section>
    </>
