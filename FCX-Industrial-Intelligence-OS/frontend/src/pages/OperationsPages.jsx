...
function OperationalTablePage({ title, subtitle, path, columns, mapRow = (row) => row, metrics }) {
...
export function AssetsPage() {
  return <OperationalTablePage title="Ativos industriais" subtitle="Inventário, criticidade e condição operacional." path="/assets"
    ...
    metrics={(rows) => [{ label: 'Total de ativos', value: rows.length }, { label: 'Críticos', value: rows.filter((x) => x.criticidade === 'CRITICAL').length, tone: 'danger' }, { label: 'Em alarme', value: rows.filter((x) => x.status === 'ALARM').length, tone: 'warning' }]} />;
}
...
export function AlarmsPage() {
  return <OperationalTablePage title="Gestão de alarmes" subtitle="Eventos ativos, reconhecidos e resolvidos." path="/alarms"
    ...
    metrics={(rows) => [{ label: 'Alarmes', value: rows.length }, { label: 'Ativos', value: rows.filter((x) => x.status === 'ACTIVE').length, tone: 'danger' }, { label: 'Críticos', value: rows.filter((x) => x.severidade === 'CRITICAL').length, tone: 'danger' }]} />;
}
...
export function TelemetryPage() {
  return <OperationalTablePage title="Telemetria" subtitle="Últimas leituras recebidas dos ativos conectados." path="/telemetry?limit=250"
    ...
    metrics={(rows) => [{ label: 'Leituras exibidas', value: rows.length }, { label: 'Ativos na janela', value: new Set(rows.map((x) => x.assetId)).size }, { label: 'Última leitura', value: rows[0]?.timestamp ? new Date(rows[0].timestamp).toLocaleTimeString('pt-BR') : '-' }]} />;
}
...
export function WorkOrdersPage() {
  return <OperationalTablePage title="Manutenção e ordens de serviço" subtitle="Planejamento e acompanhamento das intervenções técnicas." path="/work-orders"
    ...
    metrics={(rows) => [{ label: 'Ordens', value: rows.length }, { label: 'Em aberto', value: rows.filter((x) => x.status === 'OPEN').length, tone: 'warning' }, { label: 'Urgentes', value: rows.filter((x) => x.prioridade === 'URGENT').length, tone: 'danger' }]} />;
}
