...
export default function PredictivePage() {
  ...
  return <>
    ...
    {!resource.error && !resource.loading && !kpis.criticalAssets && !kpis.anomalies ? <div className="notice emptyNotice">{WAITING_FOR_DEVICES}</div> : null}
    <section className="panelGrid predictive">
      ...
      <Panel title="Anomalias detectadas" subtitle="Eventos recentes">
        <div className="eventList">{(widgets.anomalies || []).length ? widgets.anomalies.slice(0, 10).map((item, index) => <div key={`${item.assetId}-${index}`}><strong>{item.assetName}</strong><span>{item.type} · {item.severity} · {formatNumber(item.value, 2)}</span></div>) : <EmptyState />}</div>
      </Panel>
